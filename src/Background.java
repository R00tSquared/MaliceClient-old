...
public final class Background extends DrawingArea {
	...
	public Background(StreamLoader streamLoader, String s, int i)
	{
		...
		for(int k = 0; k < j - 1; k++)
			anIntArray1451[k + 1] = stream_1.getMedium();

		for(int l = 0; l < i; l++)
		{
			stream_1.currentOffset += 2;
			stream.currentOffset += stream_1.getUnsignedShort() * stream_1.getUnsignedShort();
			stream_1.currentOffset++;
		}

		anInt1454 = stream_1.getUnsigned();
		anInt1455 = stream_1.getUnsigned();
		...
		if(i1 == 1)
		{
			for(int l1 = 0; l1 < imgWidth; l1++)
			{
				for(int i2 = 0; i2 < imgHeight; i2++)
					imgPixels[l1 + i2 * imgWidth] = stream.get();
	...
	{
		if(imgWidth == anInt1456 && imgHeight == anInt1457)
			return;
		byte abyte0[] = new byte[anInt1456 * anInt1457];
		int i = 0;
		for(int j = 0; j < imgHeight; j++)
	...
	public void method359()
	{
		...
		imgPixels = abyte0;
		anInt1455 = anInt1457 - imgHeight - anInt1455;
	}
