...
final class Censor {
	...
	private static void readTldList(Stream stream)
	{
		...
		for(int j = 0; j < i; j++)
		{
			anIntArray625[j] = stream.getUnsigned();
			char ac[] = new char[stream.getUnsigned()];
			for(int k = 0; k < ac.length; k++)
				ac[k] = (char) stream.getUnsigned();
	...
	private static void method493(Stream stream, char ac[][], byte abyte0[][][])
	{
		...
		{
			char ac1[] = new char[stream.getUnsigned()];
			for(int k = 0; k < ac1.length; k++)
				ac1[k] = (char) stream.getUnsigned();

			ac[j] = ac1;
			byte abyte1[][] = new byte[stream.getUnsigned()][2];
			for(int l = 0; l < abyte1.length; l++)
			{
				abyte1[l][0] = (byte) stream.getUnsigned();
				abyte1[l][1] = (byte) stream.getUnsigned();
			}
