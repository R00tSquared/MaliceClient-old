	...
	public void method169(Stream stream)
	{
		...
		for(int j = 0; j < 10; j++)
		{
			...
			if(k == 0)
				break;
			anIntArray106[j] = k;
			anIntArray107[j] = stream.getSignedSmart();
			anIntArray108[j] = stream.getUnsignedSmart();
		}

		anInt109 = stream.getUnsignedSmart();
		anInt110 = stream.getUnsignedSmart();
		anInt113 = stream.getUnsignedShort();
		anInt114 = stream.getUnsignedShort();
