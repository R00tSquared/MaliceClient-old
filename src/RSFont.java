...
public class RSFont extends DrawingArea {
    ...
    public RSFont(boolean TypeFont, String s, StreamLoader archive) {
        ...
        for (int l = 0; l < 256; l++) {
            characterDrawXOffsets[l] = stream_1.getUnsigned();
            characterDrawYOffsets[l] = stream_1.getUnsigned();
            int i1 = characterWidths[l] = stream_1.getUnsignedShort();
            int j1 = characterHeights[l] = stream_1.getUnsignedShort();
            int k1 = stream_1.getUnsigned();
            int l1 = i1 * j1;
            fontPixels[l] = new byte[l1];
            if (k1 == 0) {
                for (int i2 = 0; i2 < l1; i2++) {
                    fontPixels[l][i2] = stream.get();
                }

            } else if (k1 == 1) {
                for (int j2 = 0; j2 < i1; j2++) {
                    for (int l2 = 0; l2 < j1; l2++) {
                        fontPixels[l][j2 + l2 * i1] = stream.get();
                    }
                        ...
                        if (effectString.startsWith(startImage)) {
                            try {
                                ...
                                int iconModY = icon.maxHeight;
									icon.drawSpriteOpacity(drawX, (drawY + baseCharacterHeight - iconModY), transparency);
                                drawX += icon.maxWidth;
    ...
    public void drawNewBasicString(String string, int drawX, int drawY, int color,
            int shadow) {
        if (string != null) {
			setColorAndShadow(color, 0x000000);
            drawBasicString(string, drawX, drawY);
        }
        if (string != null) {
			setColorAndShadow(color, shadow);
