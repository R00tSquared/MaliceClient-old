...
public class Sprite extends DrawingArea {
	...
	public void drawAdvancedSprite(int i, int j, boolean whiteAsTransp) {
		if (whiteAsTransp) {
			setTransparency(255, 0, 255);
	 ...
	 */
	public void drawMovingSprite(int finalCoordX, int finalCoordY,
			int startingX, int startingY, int speed, char coordToMove,
			boolean advancedSprite) {
		if (!startedMoving) {
			movingX = startingX;
			movingY = startingY;
			startedMoving = true;
		...
		if (!movedEnough) {
			...
			setSprite(coordToMove == 'y' ? startingX : movingX,
					coordToMove == 'x' ? startingY : movingY, advancedSprite);
		} else
			setSprite(coordToMove == 'y' ? startingX : movingX,
					coordToMove == 'x' ? startingY : movingY, advancedSprite);
	}
	...
	public void drawSprite1(int i, int j, int k) {
		...
		if (l1 > 0 && k1 > 0)
			block_copy_alpha(j1, l1, DrawingArea.pixels, myPixels, j2, k1, i2,
					k, i1);
	}

	public void block_copy_alpha(int i, int j, int ai[], int ai1[], int l,
			int i1, int j1, int k1, int l1) {
		int k;// was parameter
		int j2 = 256 - k1;
		for (int k2 = -i1; k2 < 0; k2++) {
	...
	public void rotate(int dimension, int rotation, int yPosArray[], int zoom,
			int xPosArray[], int basePosition, int yPosition, int xPosition,
			int dimension_1, int middle) {
		autoUpdate();
		try {
	...
	public static String getSpriteLocation(String fileName) {
		if (Configuration.currentTheme != 0 && Configuration.currentTheme < Configuration.customThemes.length) {
			String temp = signlink.findcachedir();
			temp = temp.replace("/ZarporCache/", "/ZarporThemes/"+Configuration.customThemes[Configuration.currentTheme]+"/"
					+ fileName);
	...
	public Sprite(StreamLoader streamLoader, String s, int i) {
		...
		stream_1.currentOffset = stream.getUnsignedShort();
		maxWidth = stream_1.getUnsignedShort();
		maxHeight = stream_1.getUnsignedShort();
		int j = stream_1.getUnsigned();
		int ai[] = new int[j];
		for (int k = 0; k < j - 1; k++) {
			ai[k + 1] = stream_1.getMedium();
			if (ai[k + 1] == 0)
				ai[k + 1] = 1;
		}
		...
		for (int l = 0; l < i; l++) {
			...
			stream.currentOffset += stream_1.getUnsignedShort()
					* stream_1.getUnsignedShort();
			stream_1.currentOffset++;
		}

		drawOffsetX = stream_1.getUnsigned();
		drawOffsetY = stream_1.getUnsigned();
		myWidth = stream_1.getUnsignedShort();
		myHeight = stream_1.getUnsignedShort();
		int i1 = stream_1.getUnsigned();
		int j1 = myWidth * myHeight;
		myPixels = new int[j1];
		if (i1 == 0) {
			for (int k1 = 0; k1 < j1; k1++)
				myPixels[k1] = ai[stream.getUnsigned()];
			setTransparency(255, 0, 255);
			return;
		}
		if (i1 == 1) {
			for (int l1 = 0; l1 < myWidth; l1++) {
				for (int i2 = 0; i2 < myHeight; i2++)
					myPixels[l1 + i2 * myWidth] = ai[stream.getUnsigned()];
			}
	...
	public void method347(int i, int j, int k, int l, int i1, int k1, int ai[],
			int ai1[]) {
		int l1 = -(j >> 2);
		j = -(j & 3);
	...
	public void method354(Background background, int i, int j) {
		...
		if (!(j1 <= 0 || i1 <= 0)) {
			method355(myPixels, j1, background.imgPixels, i1,
