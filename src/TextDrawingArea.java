...
public final class TextDrawingArea extends DrawingArea {
	...
	stream_1.currentOffset = stream.getUnsignedShort() + 4;
	int k = stream_1.getUnsigned();
	if(k > 0)
		stream_1.currentOffset += 3 * (k - 1);
	for(int l = 0; l < 256; l++) {
		anIntArray1494[l] = stream_1.getUnsigned();
		anIntArray1495[l] = stream_1.getUnsigned();
		int i1 = anIntArray1492[l] = stream_1.getUnsignedShort();
		int j1 = anIntArray1493[l] = stream_1.getUnsignedShort();
		int k1 = stream_1.getUnsigned();
		int l1 = i1 * j1;
		aByteArrayArray1491[l] = new byte[l1];
		if(k1 == 0) {
			for(int i2 = 0; i2 < l1; i2++)
				aByteArrayArray1491[l][i2] = stream.get();

			} else if(k1 == 1) {
				for(int j2 = 0; j2 < i1; j2++) {
					for(int l2 = 0; l2 < j1; l2++)
					aByteArrayArray1491[l][j2 + l2 * i1] = stream.get();
				}
			}
	...
	public int getTextWidth(String s) {
		...
		for(int k = 0; k < s.length(); k++)
			if (s.charAt(k) == '@' && (k + 4) < s.length()-1 && s.charAt(k + 4) == '@')
			{
				k += 4;
				if (k > s.length() - 1)
					k = s.length() - 1;
	...
	public void method390(int i, int j, String s, int k, int i1) {
		...
		/*if(Client.tabID == 8) {//for showing old account names
			if(RSApplet.mouseX > 550  && RSApplet.mouseX < 730 && RSApplet.mouseY > 240 && RSApplet.mouseY < 440 && Client.hoveredDisplay != null){ //9 ignore
				RSInterface.interfaceCache[5070].message = "Previously known as: "+ Client.hoveredDisplay;
			} else
				RSInterface.interfaceCache[5070].message = "                      World 1";
		}*/
	}
